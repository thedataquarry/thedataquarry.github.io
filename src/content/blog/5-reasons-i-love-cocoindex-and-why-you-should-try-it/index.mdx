...
Async execution is kept simple and Pythonic. `coco.map` applies an async function across a collection, runs the calls concurrently inside the current component, and returns an ordinary list of results. If the mapped function is decorated with `@coco.fn(memo=True)`, each call can benefit from CocoIndex memoization (compute only occurs when data changes). There is no special "map-reduce API" to learn: you just use normal Python to combine the results, such as joining formatted sections into one document.

```python
# Format each section concurrently within the current component.
