...
description: "An introduction to DSPy and its core abstractions"
...
If you're like me, you've probably seen posts recently on X raving about DSPy, but you probably put
it in the back of your mind, saying "sounds interesting... but I don't really get it". Having
spent more than a month engaging with the amazing [@DSPyOSS](https://x.com/DSPyOSS) community on X, and trying it
out on various use cases, it's finally becoming clear to me how powerful (and well-designed) it really is,
and I've decided that it's worth an entire blog post just explaining DSPy from the perspective of its abstraction philosophy.
...
_actually_ are in the prompt space, you come to the following humbling realization: there's an infinite
space of prompts out there that could potentially get you a better result. All you really
did after all that painstaking tweaking of words and word order in the prompt, was to hit upon an arbitrary
point in this space.

<Figure src={Img1} alt='In the infinite space of good prompts, this is where you probably are' />

Of course, a real prompt space isn't likely as smooth and continuous as this -- this is just to illustrate
the point. A real-world AI pipeline is a combination of several subtasks that each require their own distinct
prompts and styles (and indeed, language models themselves), so it's very likely that the prompt space
you're operating in is highly discontinuous. It's not at all clear what changes you make actually help or harm
the overall outcome. Put simply, prompts are _**not**_ a good abstraction to work with LMs.
...
As a developer learning any new framework, it helps to understand the following: a) the abstractions you're given
...
or on the whiteboard. Too high a level of abstraction, and you end up with rigid building blocks that
you wrestle with to align them to your task. Too low, and you're left writing tons
...
each day I use DSPy, it's amply clear to me that they've hit upon a great set of abstractions that
...
DSPy modules are composable, meaning that you can combine multiple modules together to create more complex modules for any task.
Importantly, modules serve as learning targets for the optimizer. As LMs and prompting strategies evolve, so
too does the module.
