---
title: "Build with BAML (1): Classification"
description: "Building LLM workflows with BAML to compare gpt-4o-mini and Gemma-3 on a gender classification task"
slug: "build-with-baml-1"
...
In the two months since I've begun using [BAML](https://boundaryml.com), it's become a
game changer in my ideation and experimentation process when working with LLMs.
As a result, I've decided to do a series of blog posts where I'll be describing how I
use BAML to build LLM workflows for a variety of tasks.
I'll do my best to illustrate how the rapid iteration cycle in BAML -- from an initial
idea to producing a result that can be tested and evaluated quickly -- is the key reason why building with BAML
is so effective, and at the same time, fun! 🚀

I hope that by the end of this post, you'll be as excited as I am to try out new ideas and get your hands dirty with LLMs,
and with BAML at the helm!
...
an emergent "reasoning" ability is observed in these models. Given the nature of this task, it's perfectly
...
and developed its own internal representations of them. Can we leverage LLMs' _memorized_ knowledge
in combination with their reasoning abilities to classify the gender of a given name?

That's the main goal of this post! We'll use the following approaches to classify the gender of a given scholar's name:
...
[here](https://github.com/prrao87/build-with-baml/blob/main/src/classification/baml_classifier_async.py).
...
if __name__ == "__main__":
    ...
    # female
```
The full code is available [here](https://github.com/prrao87/build-with-baml/blob/main/src/classification/genderapi_classifier_async.py).

## Results

It's time to compare the results between the different methods!

### Names marked as "Unknown" gender

Across the three LLMs, there were 62 names that were marked as "Unknown" by at least one of the methods.
...
The fact that `gemma-3-12b` got more names than `gemma-3-27b` correct is interesting, and we will inspect this further, using
Chain-of-Draft (CoD) prompting as shown below!

Below are some sample names that were marked as "Unknown" by each LLM.
...
confident (fewer names marked as "Unknown") than its 27B counterpart. More on that below!
...
The final experiment is arguably the most interesting. As of 2025, there have (sadly) been only 65 female Nobel laureates
...
Glad you made it this far, and I hope this was an informative read! In this post, I showed how to
formulate a gender classification task on an existing dataset. Because the dataset involves real-world historical
scholars, it was straightforward to build an evaluation suite to compare and contrast
the performance of three LLMs vs. a gender-prediction API. It's important to not take the LLM's results
at face value, however, and to do a more thorough analysis of the outputs, as shown here.

<Aside type="tip" title="Key takeaway">
A reasoning-capable LLM could potentially perform better than a rule-based system or ML model at classification tasks, because it has
...
As I've begun using BAML more and more in my LLM workflows, I've come to appreciate the speed with which
I can easily test prompts, evaluate the results, and iterate with confidence. The BAML UI makes the prompts themselves totally
transparent, and by inspecting the raw LLM outputs alongside the final outputs, it's super straightforward
to analyze the chain-of-thought (or chain-of-draft) reasoning of each model. This leads to a _much_ deeper
understanding of the strengths and weaknesses of each model, as shown in the case of `gemma-3`'s 12b and 27b variants.

If you're coming from other frameworks, you're likely used to testing LLM _outputs_ via tools like pytest or jest. In BAML,
you test _the prompts themselves_, and you have access to the LLM's raw outputs. Prompts are functions with unit tests.
Every prompt in BAML requires at least one unit test, and
you can very quickly create tests for numerous edge cases in your domain. Then, when you find that a shiny new model just
came out, simply swap out the client in BAML (as easily as shown above), and rerun your tests to see how the new model does.
It's that simple! In my experience, the BAML way captures the issues in
prompts well in advance of application-driven test suites downstream, increasing iteration speed.
...
As BAML continues to evolve and grow, there are more and more kinds of problems I see myself solving with it.
To learn more and engage with the awesome BAML community, join BAML [Discord](https://discord.gg/BTNBeXGuaS),
and give them a star [on GitHub](https://github.com/BoundaryML/baml)! 

Have fun building with BAML and the latest LLMs! 🚀

## Code & acknowledgements

To reproduce the experiments and evaluations shown in this post, check out this
[GitHub repo](https://github.com/prrao87/build-with-baml/tree/main/src/classification).
...
I highly recommend joining the YourTechBud Studio [Discord](https://discord.gg/Ggb4sssHEv) to chat with more BAML users
